<template>
  <form class="listing-form" @submit.prevent="onSubmit">
    ...
    <div class="field-group">
      ...
      <p v-if="errors.watering" class="field-error">{{ errors.watering }}</p>
      <div class="chip-group">
        <button
    ...
    <!-- Wysokość -->
    <div class="field-group">
      <label class="field-label">Wysokość <span class="optional">(opcjonalne)</span></label>
      <div class="chip-group">
        <button
    ...
    <!-- Trudność -->
    <div class="field-group">
      <label class="field-label">Trudność uprawy <span class="optional">(opcjonalne)</span></label>
      <div class="chip-group">
        <button
          ...
          @click="form.difficulty = form.difficulty === opt.value ? '' : opt.value"
        >
          <Icon :name="opt.icon" />
          {{ opt.label }}
        </button>
    ...
    <!-- Przyjazne dla zwierząt (tylko add) -->
    <div v-if="mode === 'add'" class="field-group">
      <label class="field-label">Przyjazne dla zwierząt <span class="optional">(opcjonalne)</span></label>
      <div class="chip-group">
        <button
          type="button"
          class="chip"
          :class="{ active: form.petFriendly === true }"
          @click="form.petFriendly = form.petFriendly === true ? null : true"
        >
          <Icon name="mdi:paw" />
          Tak
        </button>
        <button
          ...
          @click="form.petFriendly = form.petFriendly === false ? null : false"
        >
    ...
    <!-- Zdjęcia (tylko add) -->
    <div v-if="mode === 'add'" class="field-group span-full">
    ...
    <p v-if="serverError" class="server-error span-full">{{ serverError }}</p>
    ...
    >
      {{ mode === 'add' ? 'Opublikuj ogłoszenie' : 'Zapisz zmiany' }}
    </VButton>
  </form>
</template>
...
const emit = defineEmits<{
  submit: [data: ListingFormData & { imageUrls: string[] }];
}>();

const form = reactive<ListingFormData>({
  title: "",
  description: "",
  city: "",
  type: "",
...
watch(
  ...
  (d) => {
    ...
    Object.assign(form, {
      ...
      city: d.city ?? "",
      type: d.type ?? "",
      deliveryMode: d.deliveryMode ?? "PICKUP",
      watering: d.watering ?? "",
      light: d.light ?? "",
      position: d.position ?? "",
      height: d.height ?? "",
      difficulty: d.difficulty ?? "",
      petFriendly: d.petFriendly ?? null,
    });
  },
  { immediate: true },
);
...
async function uploadFile(file: File) {
  ...
  previews.value.push({ previewUrl: URL.createObjectURL(file), uploadedUrl: null, uploading: true });
  try {
...
const deliveryOptions = [
  ...
  { value: "SHIPPING", label: "Wysyłka", icon: "mdi:truck-outline" },
  { value: "BOTH", label: "Oba", icon: "mdi:swap-horizontal" },
];
...
const isDirty = computed(() => {
  if (props.mode !== "edit" || !props.initialData) return true;
  ...
  return (
    ...
    form.city !== (d.city ?? "") ||
    form.type !== (d.type ?? "") ||
    form.deliveryMode !== (d.deliveryMode ?? "PICKUP") ||
    form.watering !== (d.watering ?? "") ||
    form.light !== (d.light ?? "") ||
    form.position !== (d.position ?? "") ||
    form.height !== (d.height ?? "") ||
    form.difficulty !== (d.difficulty ?? "") ||
    form.petFriendly !== (d.petFriendly ?? null)
  );
});
...
function validate() {
  let ok = true;
  errors.title = form.title.trim().length < 3 ? "Nazwa musi mieć co najmniej 3 znaki" : null;
  if (errors.title) ok = false;
  errors.description = form.description.trim().length < 10 ? "Opis musi mieć co najmniej 10 znaków" : null;
...
function onSubmit() {
  if (!validate()) return;
  const imageUrls = previews.value.filter((p) => p.uploadedUrl).map((p) => p.uploadedUrl as string);
  emit("submit", { ...form, imageUrls });
}
</script>
